...
const insertChatHistory = async (userid, roomid, chatcontent, createddate, callback) => {
    const db = new sqlite3.Database(databasepath, (err) => {
        const query = `
...
const createNewRoom = async (userid, roomname, passcode) => {
    return new Promise((resolve, reject) => {
        ...
        const db = new sqlite3.Database(databasepath, (err) => {
            ...
            db.run(query, [userid, roomname, todaydate, encryptedpasscode], function (err) {
                if (err) {
...
const main = async () => {
    try {
        ...
        await insertChatHistory(userId1, roomid, 'hello world', todaydate, (err, result) => {
            if (err) {
...
app.post('/model/createnewroom', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, message: 'Create New Room successfully', userId });
    } catch (err) {
        console.error('Error inserting user:', err.message);
...
app.post('/api/insertchathistory', async (req, res) => {
    ...
    await insertChatHistory(userid, roomid, chatcontent, createddate, (err, result) => {
        if (err) {
            console.error('Error inserting chat history:', err);
            return res.status(500).json({ error: 'Failed to insert chat history.' });
        }
        res.status(200).json({ message: 'Chat history added successfully.', id: result.id });
    });
});
